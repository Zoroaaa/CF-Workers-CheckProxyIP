...
let 临时TOKEN, 永久TOKEN;
// https://ipcheck.xxx.workers.dev/check?proxyip=xxx/token=xxx  https://ipcheck.xxx.workers.dev/check?proxyip=xxx&token=xxx 
export default {
  async fetch(request, env, ctx) {
    ...
    // 如果路径第一个段是TOKEN，则设置验证标志并移除该段
    let 已验证 = false;
    let 新路径 = path;
    let 路径TOKEN = null;
    
    if (pathSegments.length > 0 && (pathSegments[0] === 永久TOKEN || pathSegments[0] === 临时TOKEN)) {
      已验证 = true;
      路径TOKEN = pathSegments[0];
      // 重新构建路径（移除TOKEN部分）
      新路径 = '/' + pathSegments.slice(1).join('/');
      if (新路径 === '/') 新路径 = '/'; // 确保根路径正确
    }

    // 使用新路径进行后续判断
    const 实际路径 = 新路径.toLowerCase();

    // 原有的接口逻辑，增加TOKEN验证
    if (实际路径 === '/check') {
      // 如果设置了TOKEN且未通过路径验证，需要验证查询参数TOKEN
      if (env.TOKEN && !已验证) {
      ...
      // 根路径或首页访问
      if (env.TOKEN && 已验证) {
        // 如果通过路径TOKEN访问，返回正常的HTML页面
        return await HTML(hostname, 网站图标, 路径TOKEN);
      }
      ...
      if (envKey) {
        ...
        // 如果设置了TOKEN但未通过验证，返回nginx页面
        return new Response(await nginx(), {
          headers: {
            'Content-Type': 'text/html; charset=UTF-8',
          },
        });
      } else if (实际路径 === '/favicon.ico') {
        return Response.redirect(网站图标, 302);
      }
      // 直接返回HTML页面
      return await HTML(hostname, 网站图标, null);
    }
  }
};
...
async function HTML(hostname, 网站图标, pathToken = null) {
  // 首页 HTML
...
<body>
  ${pathToken ? `<div class="token-badge">🔑 TOKEN验证通过</div>` : ''}
  ...
  <div class="container">
    <header class="header">
      ...
      ${pathToken ? `<div class="subtitle">🔒 安全模式 - 路径TOKEN验证已启用</div>` : ''}
    </header>
    ...
    <div class="card">
      <div class="form-section">
        ...
        ${!pathToken && env.TOKEN ? `
          <div style="margin-top: 12px; padding: 12px; background: #fff3cd; border-radius: 8px; border-left: 4px solid #f39c12;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="color: #856404; font-size: 16px;">⚠️</span>
              <span style="color: #856404; font-size: 14px;">TOKEN验证已启用，请通过 <code>/你的TOKEN</code> 路径访问完整功能</span>
            </div>
          </div>
        ` : ''}
      </div>
    ...
    <div class="api-docs" style="margin-top: 50px;">
      ...
      <div class="code-block">
curl "https://${hostname}${pathToken ? '/' + pathToken : ''}/check?proxyip=1.2.3.4:443"
      </div>
...
https://${hostname}/check?proxyip=1.2.3.4&token=<span class="highlight">${pathToken || '你的TOKEN'}</span>
    ...
    <div class="footer">
      ...
      ${pathToken ? '<p style="margin-top: 4px; opacity: 0.6;">🔒 当前会话已通过TOKEN验证</p>' : ''}
    </div>
  </div>
  ...
  <script>
    ...
    const pathToken = '${pathToken || ''}'; // 从后端注入的TOKEN
    
    // 计算时间戳的函数
    function calculateTimestamp() {
      const currentDate = new Date();
      return Math.ceil(currentDate.getTime() / (1000 * 60 * 31)); // 每31分钟一个时间戳
    }
    ...
    // 通用的API调用函数，自动处理TOKEN
    async function callAPI(endpoint, params = {}) {
      let url = endpoint;
      
      // 如果存在路径TOKEN，自动添加到参数中
      if (pathToken) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (!autoCheckValue && currentPath.length > 1) {
        ...
        // 如果第一个段是TOKEN，则检查第二个段
        if (pathSegments.length > 1 && (pathSegments[0] === pathToken || !pathToken)) {
          const pathContent = pathSegments[1];
          // 检查路径是否为有效的代理IP格式
          if (isValidProxyIPFormat(pathContent)) {
            autoCheckValue = pathContent;
          }
        } else if (pathSegments.length === 1) {
          const pathContent = pathSegments[0];
          // 检查路径是否为有效的代理IP格式（且不是TOKEN）
          if (isValidProxyIPFormat(pathContent) && pathContent !== pathToken) {
            autoCheckValue = pathContent;
          }
        }
      }
    ...
    async function checkSingleIP(proxyip, resultDiv) {
      const data = await callAPI('./check', { proxyip: proxyip });
    ...
    async function checkIPWithIndex(ip, port, index) {
      try {
        ...
        if (ipCheckResults.has(cacheKey)) {
          ...
          // 调用API检查IP状态
          result = await callAPI('./check', { proxyip: cacheKey });
          // 缓存结果
    ...
    async function getIPInfo(ip) {
      try {
        ...
        const data = await callAPI('./ip-info', { ip: cleanIP });
        return data;
      } catch (error) {
        return null;
      }
